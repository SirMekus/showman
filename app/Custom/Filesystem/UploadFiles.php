...
namespace App\Custom\Filesystem;
...
class UploadFiles
{
	...
	//public $docParentFolder = "public/uploads";
	public $sub_folder;
	public $path_to_file;
	...
	public function __construct($sub_folder="movies", $user_defined_valid_mimes=[], $override_mimes = false)
	{
		...
		$this->docParentFolder = config("settings.file.directory");

		$this->request = request();
		
		$this->sub_folder = $sub_folder;
		
		$this->path_to_file = $this->docParentFolder.'/'.$sub_folder.'/';
    ...
    {
		if(!$this->request->hasFile($file_upload_name))
		{
			if($ignore == true)
			{
				return;
			...
			{
				abort(422, "Please upload at least one image");
			}
		}
		
	    $file = $this->request->file($file_upload_name);
		//dd(is_array($file));
		if(is_array($file))
		{
            ...
            {
				abort(422, "<p class='alert alert-danger'>Exceeded maximum number of products to upload. You can only upload ".$this->max_no_of_file_to_upload." maximum number of products</p>");
            }   
		    ...
		    for($i=0;$i<count($file);$i++)
            {
				if(!empty($file[$i]->getClientOriginalName()))
				{
					...
					{
						abort(422, "<p class='alert alert-danger'>Couldn't upload this file</p>");
					}
				}
		...
		else
		{
			if(!empty($file->getClientOriginalName()))
			{
			    ...
			    if(!in_array($file_type, $this->valid_mimes))
				{
					abort(422, "<p class='alert alert-danger'>Couldn't upload this fil</p>");
			    }
		    
		        if($file->getSize() > $this->max_file_upload_size)//if file is larger than a specified size.
		        {
					abort(422, "<p class='alert alert-danger'>".$file->getClientOriginalName()." is too large. Please make sure it's less than or equal to 2mb</p>");
	            }
